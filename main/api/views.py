...
def get_FahrradtypByDateTimeForPie(request):
    try:
        ...
        data = (
            Report.objects.filter(createdDay__gte=startDate, createdDay__lt=endDate)
            .values("typeOfBike")
...
def get_SchadenByTimeForLine(request):
    ...
    data = Report.objects.filter(
        createdDay__gte=startDate, createdDay__lt=endDate
    ).values("beginHour", "endHour", "beginDay", "endDay", "damage")
...
def get_geojson(request):
    ...
    try:
        ...
        data = (
            Report.objects.filter(createdDay__gte=startDate, createdDay__lt=endDate)
            .values("lor")
            .annotate(count=Count("createdDay"))
        )
        ...
        with open("./data/lor_" + mapType + "_min.geojson") as json_file:
            ...
            if steps >= 1:
                ...
                geojson["proportions"] = [
                    round(x * summe) for x in arange(0, 0.15, 0.0225)
                ]
...
def get_DelictByDateTimeForBar(request):
    try:
        ...
        data = (
            Report.objects.filter(createdDay__gte=startDate, createdDay__lt=endDate)
            .values("delict", "createdDay")
            .annotate(count=Count("delict"))
            .order_by("delict")
        )
...
def get_TriesByDateTimeForBar(request):
    try:
        ...
        data = (
            Report.objects.filter(createdDay__gte=startDate, createdDay__lt=endDate)
            .values("tryBike", "createdDay")
            .annotate(count=Count("tryBike"))
            .order_by("tryBike")
        )
